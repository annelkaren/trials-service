...
public class CarpetaMigracionService {
    ...
    public void createPiezaConDocumentos(Carpeta carpeta, List<MovimientosMigracionRecord> piezas){
        ...
        piezas.forEach(pieza -> {
            ...
            TipoPieza tipoPieza = tipoPiezaRepository.findByClave(tipoPiezaString)
                .orElseThrow(() -> new NotFoundException("La clave de la pieza no existe", tipoPiezaString));
...
public String tipoPieza(String cu) {
    ...
    int len = cu.length();
    if (len != 16) {
        throw new IllegalArgumentException("cu debe tener exactamente 16 caracteres; recibido " + len + ": '" + cu + "'");
    }
    ...
    if (!cu.matches("\\d{12}[A-Za-z]{2}\\d{2}")) {
        throw new IllegalArgumentException("Formato de pieza (cu) inválido. Se esperaba 12 dígitos, 2 letras, 2 dígitos.");
    }
