...
public interface DocumentoRepository extends JpaRepository<Documento, Integer>, SecuenciaRepositoryCustom {
    ...
    @Query("""
                ...
                coalesce(pd.apellidoMaterno, '')
                            ...
                            AND (
                                (LOWER(:tipoParte) = 'ambos' AND (lower(tp.nombre) LIKE '%actor%' OR lower(tp.nombre) LIKE '%demandado%'))
                                OR
                                (LOWER(:tipoParte) = 'actor' AND lower(tp.nombre) LIKE '%actor%')
                                OR
                                (LOWER(:tipoParte) = 'demandado' AND lower(tp.nombre) LIKE '%demandado%')
                                OR
                                (LOWER(:tipoParte) = 'otros' AND lower(tp.nombre) NOT LIKE '%actor%' AND lower(tp.nombre) NOT LIKE '%demandado%')
                            )
                        """)
