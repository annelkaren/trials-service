...
public class DocumentoService {
    ...
    private Pageable translateBandejaSalidaPageable(Pageable pageable) {
        ...
        for (Sort.Order o : incoming) {
            ...
            Sort s = switch (p) {
                ...
                case "expediente" -> JpaSort.unsafe("COALESCE(c.expediente, doc_carpeta.expediente, '')");
                case "materia" -> JpaSort.unsafe("COALESCE(matc.nombre, matd.nombre, '')");
                case "organoJurisdiccional", "juzgado" -> JpaSort.unsafe("COALESCE(jc.nombre, jd.nombre, '')");
                case "fechaRegistro" -> JpaSort.unsafe("COALESCE(c.audit.fechaAlta, doc_carpeta.audit.fechaAlta)");
                case "estatus" -> JpaSort.unsafe("COALESCE(c.estatus, doc_carpeta.estatus)");
                case "selloEstatus" -> JpaSort.unsafe("COALESCE(c.selloEstatus, doc_carpeta.selloEstatus)");
    ...
    public DocumentoResponseRecord getDemandaById(Integer id) {
        ...
        CarpetaDetalle carpetaDetalle = carpetaDetalleRepository
                .findByCarpetaId(documento.getCarpeta().getId());
        String ultimoDomicilioFamiliar = carpetaDetalle != null ? carpetaDetalle.getUltimoDomicilioFamiliar()
                : null;
    ...
    public DocumentoRecord createApelacion(ApelacionRecord apelacionRecord) {
        ...
        log.info(
                ...
                documento.getData() != null ? documento.getData().getApelacionAntecedenteCarpeta()
                        : null);
    ...
    @Transactional
    public CarpetaResponseRecord createExpedienteSinAntecedentes(
            ExpedienteSinAntecedentesRecord expedienteSinAntecedentesRecord) {
        ...
        Carpeta carpeta = new Carpeta()
                ...
                .setExpediente(expedienteSinAntecedentesRecord.expediente() + "/"
                        + expedienteSinAntecedentesRecord.year())
                .setEstatus(EstadoCarpeta.ASIGNADO)
                .setSelloEstatus(SelloEstatus.VALIDO)
                .setFechaAsignacion(LocalDateTime.now())
                .setPersona(persona)
    ...
    public Page<DocumentoVisitaduriaRecord> findDocumentosVisitaduria(
        ...
        try {
        List<DocumentoVisitaduriaRecord> rawResults = documentoRepository.findDocumentosVisitaduria(
                fechaInicio, fechaFin, juzgadoId, tipoAcuerdo, key
        );

        Map<Integer, List<DocumentoVisitaduriaRecord>> groupedById = rawResults.stream()
                .collect(Collectors.groupingBy(DocumentoVisitaduriaRecord::id));

        List<DocumentoVisitaduriaRecord> aggregatedList = groupedById.values().stream()
                .map(rows -> {
                    DocumentoVisitaduriaRecord firstRow = rows.get(0);

                    String emails = rows.stream()
                            .map(DocumentoVisitaduriaRecord::emailsNotificados)
                            .filter(Objects::nonNull)
                            .distinct()
                            .collect(Collectors.joining(", "));

                    if (emails.isEmpty()) {
                        emails = "Sin notificar";
                    }

                    return new DocumentoVisitaduriaRecord(
                            firstRow.id(),
                            firstRow.expediente(),
                            firstRow.idCarpeta(),
                            firstRow.fechaAlta(),
                            firstRow.nombreCompletoPersonal(),
                            emails,
                            firstRow.tipoAcuerdo()
                    );
                })
                .toList();
        ...
        List<DocumentoVisitaduriaRecord> pageContent = List.of();
        if (start <= end) {
            pageContent = aggregatedList.subList(start, end);
        }

        return new PageImpl<>(pageContent, pageable, aggregatedList.size());
        } catch (Exception ex) {
    ...
    public Page<BandejaTurnoRecord> getAllTurno(String key, Integer juzgadoId, Integer secretarioId,
                                                LocalDateTime fechaInicial, LocalDateTime fechaFinal,
                                                Pageable pageable) {


        key = (key != null) ? key.toLowerCase() : "";
        Page<TurnoRecord> pageDeTurnos = movimientoRepository.getAllBandejaTurno(key, juzgadoId, secretarioId, fechaInicial, fechaFinal, pageable);

        List<BandejaTurnoRecord> listaModificada = pageDeTurnos.getContent().stream()
                .map(turno -> {

                    String fechaFormateada = turno.fechaAsignacion().format(FORMATO_FECHA);
                    String horaFormateada = turno.fechaAsignacion().format(FORMATO_HORA);

                    // String estadoFormateado = mapearEstado(turno.estaEnJuzgado());

                    Integer dias = parseDias(turno.dias());

                    String vencimientoFormateado = calcularVencimiento(turno.fechaAsignacion(), dias);

                    String fechaCancelado = "N/A";

                    if (turno.fechaSiguienteMovimiento() != null) {

                        fechaCancelado = turno.fechaSiguienteMovimiento().format(FORMATO_FECHA);
                    }

                    String tipoEntradaFormateado = mapearTipoEntrada(
                            turno.tipoCarpetaOrdinal(),
                            turno.tipoDocumentoOrdinal()
                    );



                    return new BandejaTurnoRecord(
                            turno.id(),
                            turno.carpetaId(),
                            turno.expediente(),
                            fechaFormateada,
                            ...
                            // fechaCancelado,
                            turno.nombreSecretario(),
                            dias,
                            tipoEntradaFormateado,
                            fechaCancelado
                    );
                }).toList();

        return new PageImpl<>(listaModificada, pageable, pageDeTurnos.getTotalElements());
    }

    private Integer parseDias(String diasString) {
        if (diasString == null || diasString.isEmpty()) {
            return 0;
        }
        try {

            String num = diasString.replace("d", "").trim();
    ...
    private String calcularVencimiento(LocalDateTime fechaAsignacion, Integer dias) {
        if (fechaAsignacion == null || dias == null) {
            return "";
        }

        LocalDate fechaVencimiento = fechaAsignacion.toLocalDate().plusDays(dias);

        return fechaVencimiento.format(FORMATO_FECHA);
    }

    private String mapearTipoEntrada(Integer tipoCarpetaOrdinal, Integer tipoDocumentoOrdinal) {
        try {
            if (tipoCarpetaOrdinal != null) {
                TipoCarpeta tipo = TipoCarpeta.values()[tipoCarpetaOrdinal];
                return tipo.getEtiqueta();
            }

            if (tipoDocumentoOrdinal != null) {
                TipoDocumento tipo = TipoDocumento.values()[tipoDocumentoOrdinal];
                return tipo.getEtiqueta();
            }
        } catch (ArrayIndexOutOfBoundsException e) {
            System.err.println("Error: El ordinal del enum no es válido: " + e.getMessage());
    ...
    public Page<CarpetaVisitaduriaRecord> findCarpetasVisitaduria(
        ...
        Integer tipoDocumento = (tipoEntrada != null && tipoEntrada.equalsIgnoreCase("PROMOCION")) ? TipoDocumento.PROMOCION.ordinal() : null;
        if (tipoDocumento == null) {
            ordinal = (tipoEntrada == null) ? null : TipoCarpeta.fromEtiqueta(tipoEntrada).ordinal();
        }
        Boolean soloPromociones = tipoDocumento != null;
        Page<CarpetaVisitaduriaRecord> page = carpetaRepository.findCarpetasVisitaduria(fechaInicio, fechaFin, juzgadoId, ordinal, tipoDocumento, soloPromociones, key, pageable);
        List<CarpetaVisitaduriaRecord> content = page.getContent().stream()
                .map(record -> {
                    String descripcion = null;
                    LocalDateTime fechaRecepcion = null;
                    Movimiento mov = null;
                    if (record.descripcion().equalsIgnoreCase("DOCUMENTO")){
                        descripcion = "Promoción";
                        fechaRecepcion = movimientoRepository.findPrimeraFechaRecepcionByDocumentoId(record.id());
                        mov = movimientoRepository.findFirstByDocumentoIdAndEstadoOrderByFechaAsignacionAsc(record.id(), "TURNADO");
                    } else {
                        descripcion = TipoCarpeta.values()[Integer.valueOf(record.tipo())].getEtiqueta();
                        fechaRecepcion = movimientoRepository.findPrimeraFechaRecepcionBycarpetaId(record.id());
                        mov = movimientoRepository.findFirstByCarpetaIdAndEstadoOrderByFechaAsignacionAsc(record.id(), "TURNADO");
                    }
                    return new CarpetaVisitaduriaRecord(
                            record.id(),
                            record.expediente(),
                            fechaRecepcion,
                            (mov != null) ? mov.getFechaAsignacion() : null,
                            descripcion,
                            record.descripcion(),
                            (mov != null) ? convertirADias(mov.getDuracion()) : null
                    );
                })
                .toList();
    ...
    private Integer convertirADias(String duracion) {
        if (duracion == null || duracion.isBlank()) {
            return 0;
        }

        duracion = duracion.trim().toLowerCase();

        if (duracion.endsWith("d")) {
            return Integer.parseInt(duracion.replace("d", ""));
        }

        if (duracion.endsWith("h")) {
            int horas = Integer.parseInt(duracion.replace("h", ""));
            return horas / 24; // división entera
        }
