...
public class JuzgadosMigracionReader {
    ...
    public List<JuzgadosMigracionRecord> buscarTodos() {
        ...
        Optional<JuzgadosMigracionRecord> juzgadosOptional = juzgadosRepository.findJuzgadoByCodigo(claveJuzgado);

        if (juzgadosOptional.isEmpty()) {
            throw new NotFoundException("Clave de juzgado no encontrada", "claveJuzgado");
        }

        return List.of(juzgadosOptional.get());
    }

   
    public JuzgadosMigracionRegistroRecord findByCodigo(String codigo) {
