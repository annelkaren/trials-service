...
class AcuerdoResourceTest {
        ...
        void obtenerTipoPartesAcuerdo() throws Exception {
                ...
                mockMvc.perform(
                                get("/api/workflow/documentos/obtenerTipoPartesAcuerdo/{carpetaId}/{tipoParte}", 1,
                                                "actor")
                                                .contentType(MediaType.APPLICATION_JSON))
                                .andExpect(status().isOk());
        }
        ...
        void obtenerPromocionesTest() throws Exception {
                List<AcuerdoPromocionesRecord> acuerdoPromocionesRecord = AcuerdoRecordSetUp
                                .createAcuerdoPromocionesRecord();

                given(acuerdosService.obtenerPromociones(anyInt(), anyInt(), anyString()))
                                .willReturn(acuerdoPromocionesRecord);

                mockMvc.perform(
                                get("/api/workflow/documentos/obtenerPromociones/{carpetaId}/{documentoId}/{tipoDocumento}",
                                                1, 1, "ACUERDO")
                                                .contentType(MediaType.APPLICATION_JSON))
                                .andExpect(status().isOk());
        }
