...
class AudienciaPruebasResourceTest {
        ...
        void updateDesistimientoAdmision_ShouldReturnOk() throws Exception {
                ...
                Mockito.doNothing().when(audienciasPruebasService).pachDesistimientoAdmision(Mockito.anyString(),
                                Mockito.anyInt());
        ...
        void getAllByAudiencia_ShouldReturnDetails() throws Exception {
                ...
                List<DetallesPruebasRecord> detallesPruebasRecords = List.of(
                                new DetallesPruebasRecord(1, "PersonaA", "Prueba A", "Descripción A", "N/A",
                                                DesistimientoAdmision.DESISTIMIENTO),
                                new DetallesPruebasRecord(2, "PersonaB", "Prueba B", "Descripción B", "N/A",
                                                DesistimientoAdmision.ADMISION));

                Page<DetallesPruebasRecord> pageMock = new PageImpl<>(detallesPruebasRecords);

                Mockito.when(audienciasPruebasService.getAudienciaPruebasByAudiencia(Mockito.anyInt(),
                                Mockito.any(Pageable.class)))
                                .thenReturn(pageMock);

                mockMvc.perform(get("/api/workflow/audiencias-pruebas/{id}", id))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.content.length()").value(2)) // Verificamos que el contenido
                                                                                    // tiene 2 elementos
