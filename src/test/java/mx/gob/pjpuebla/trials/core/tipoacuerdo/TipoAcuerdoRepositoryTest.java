...
class TipoAcuerdoRepositoryTest extends AuditConfigTest {
        ...
        void findByMateriaId() {
                ...
                Stream.of(100, 150, 200, 300).forEach(materiaId -> {
                        ...
                        assertTrue(tipoAcuerdos.stream().allMatch(t -> t.getMateria().getId().equals(materiaId)),
                                        "Todos los TipoAcuerdo deben tener el materiaId esperado para materiaId "
                                                        + materiaId);
                });
        }
        ...
        void findByMateriaIdAndTipoSistemaId() {
                ...
                Stream.of(100, 101).forEach(tipoSistemaId -> {
                        List<TipoAcuerdo> tipoAcuerdos = tipoAcuerdoRepository.findByMateriaIdAndTipoSistemaId(
                                        materia.getId(),
                                        tipoSistemaId);
                        assertNotNull(tipoAcuerdos, "La lista no debe ser nula para tipoSistemaId " + tipoSistemaId);
                        assertFalse(tipoAcuerdos.isEmpty(),
                                        "La lista no debe estar vacía para tipoSistemaId " + tipoSistemaId);
                        assertTrue(tipoAcuerdos.stream().allMatch(t -> t.getMateria().getId().equals(materia.getId()) &&
                                        t.getTipoSistema().getId().equals(tipoSistemaId)),
                                        "Todos los TipoAcuerdo deben tener el materiaId y tipoSistemaId esperados para tipoSistemaId "
                                                        + tipoSistemaId);
                });
        }
}