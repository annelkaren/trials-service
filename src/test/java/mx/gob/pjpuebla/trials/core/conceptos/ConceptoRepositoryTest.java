...
class ConceptoRepositoryTest extends AuditConfigTest {
    ...
    void findAllConceptos_shouldReturnResultsBasedOnCriteria() {
        ...
        Page<Concepto> result = conceptoRepository.findAllConceptos(key, estados, pageable);
        assertThat(result).isNotEmpty();
        assertThat(result.getContent()).allMatch(concepto ->
                concepto.getEstado().equals(Estado.ACTIVE) || concepto.getEstado().equals(Estado.INACTIVE));
        assertThat(result.getContent()).allMatch(concepto ->
                concepto.getNombre().toLowerCase().contains(key) || (concepto.getTipoJuicio() != null
